...
def device_uses_today(records, device_id: str, session_date: str) -> int:
    count = 0
    for r in records:
        if r.get("sessionDate") != session_date:
            continue
        for p in ("p1", "p2"):
            slot = r.get(p)
            if slot and slot.get("deviceId") == device_id:
                count += 1
    return count
...
def device_check(body: DeviceCheckBody):
    with WRITE_LOCK:
        data = load_data()
    for r in data["records"]:
        if r.get("sessionDate") != body.sessionDate:
            continue
        p1dev = (r.get("p1") or {}).get("deviceId")
        p2dev = (r.get("p2") or {}).get("deviceId")
        if body.deviceId in (p1dev, p2dev):
            filled = [p for p in ("p1", "p2") if r.get(p)]
            return {
                "found": True,
...
def checkin(body: CheckinBody, request: Request):
    ...
    with WRITE_LOCK:
        ...
        # Anti-proxy-punching: this exact phone may only be the device-of-record
        # for two period check-ins per session date, regardless of whose name.
        if device_uses_today(data["records"], body.deviceId, body.sessionDate) >= 2:
            return {"status": "device_limit"}

        existing = next(
            (r for r in data["records"] if r["fullName"] == full_name and r["sessionDate"] == body.sessionDate),
            None,
        )
        ...
        if existing:
            if existing.get(body.period):
                return {"status": "duplicate"}
            existing[body.period] = slot
            save_data(data)
            return {"status": "ok", "record": existing, "period": body.period}
        ...
        record = {
            ...
            "p1": slot if body.period == "p1" else None,
            "p2": slot if body.period == "p2" else None,
        }
        ...
        save_data(data)
        return {"status": "ok", "record": record, "period": body.period}
